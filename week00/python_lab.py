...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list = [(i,j,k) for i in S for j in S for k in S if sum([i,j,k])==0 and abs(i) + abs(j) + abs(k) != 0]
...
first_of_tuples_list = [(i,j,k) for i in S for j in S for k in S if sum([i,j,k])==0 and abs(i) + abs(j) + abs(k) != 0][0]
...
range_and_zip = zip(range(6), ['A','B','C','D','E'])
...
representation_dict = { d: (d // (base * base), d // base % base, d % base * base**0) for d in digits }
...
listdict2dict = { name:id2salary[i] for (i, name) in zip(id2salary, names) }



## 26: (Task 26) Procedure nextInts
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def nextInts(L): return list(n+1 for n in L)



## 27: (Task 27) Procedure cubes
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def cubes(L): return list(n**3 for n in L)
...
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def dict2list(dct, keylist): return list(dct[k] for k in keylist)
